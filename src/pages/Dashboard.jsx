...
export default function Dashboard() {
  ...
  // Role guard — only seller/admin allowed
  useEffect(() => {
    if (!user) return;
  ...
  return (
    <div className="md:flex md:min-h-[calc(100vh-4rem)]">
      {/* Sidebar handles nav on md+; it also renders its own mobile drawer */}
      ...
      <div className="flex-1 min-w-0">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-10">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1 text-[11px] text-ink/55 dark:text-cream/55 font-jakarta flex items-center gap-2 flex-wrap">
                  <span>
                    Role{" "}
                    <strong className="text-ink/75 dark:text-cream/75">
                      {user.role}
                    </strong>
                  </span>
          ...
          {/*
            Mobile nav is handled by <Sidebar />'s built-in hamburger drawer;
            the old inline tab bar was removed to avoid duplicate nav UI.
            Desktop (md+) also uses the Sidebar.
          */}
          ...
          <div className="mt-6">
            {tab === "overview" && (
              <div className="grid md:grid-cols-4 gap-3">
                ...
                <KpiCard
                  ...
                  tone="from-mint to-butter text-ink"
                />

                <div className="md:col-span-2 rounded-2xl bg-white/80 dark:bg-ink/80 border border-ink/5 dark:border-cream/10 p-4">
                  ...
                  {orders.slice(0, 6).map((o) => (
                    <Link
                      to={`/order/${o._id}`}
                      key={o._id}
                      className="flex items-center gap-3 p-2 rounded-lg hover:bg-peach/40 dark:hover:bg-ink/60 transition"
                    >
                      <img
                        src={o.items[0]?.image}
                        alt=""
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-ink/45 dark:text-cream/45 mt-0.5">
            ...
            {tab === "orders" && (
              <div className="rounded-2xl bg-white/80 dark:bg-ink/80 border border-ink/5 dark:border-cream/10 divide-y divide-ink/5 dark:divide-cream/10 overflow-hidden">
                {orders.length === 0 && (
                ...
                {orders.map((o) => (
                  <Link
                    to={`/order/${o._id}`}
                    key={o._id}
                    className="flex items-center gap-3 p-3 hover:bg-peach/40 dark:hover:bg-ink/60 transition"
                  >
                    <img
                      src={o.items[0]?.image}
                      alt=""
            ...
            {tab === "products" && (
              <div>
                ...
                {productsError && (
                  <div className="mb-3 rounded-xl bg-coral/10 border border-coral/30 px-3 py-2 text-[11px] font-jakarta text-coral">
                    {productsError}
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {products.map((p) => (
                    <div
                      key={p._id}
                      className="rounded-xl bg-white/80 dark:bg-ink/80 border border-ink/5 dark:border-cream/10 overflow-hidden group"
                    >
                      <Link
                        to={`/product/${p._id}`}
                      ...
                      <div className="p-3">
                        ...
                        <div className="mt-2 flex items-center gap-1.5">
                          <button
                            onClick={() => openEdit(p)}
                            className="flex-1 inline-flex items-center justify-center gap-1 rounded-full bg-cream dark:bg-ink/60 border border-ink/10 dark:border-cream/10 text-ink dark:text-cream text-[11px] font-jakarta font-semibold px-2.5 py-1 hover:border-ink/30 transition"
                          >
                  ...
                  {products.length === 0 && (
                    <div className="col-span-full text-center py-10 rounded-2xl bg-white/60 dark:bg-ink/60 border border-dashed border-ink/15 dark:border-cream/15">
            ...
            {tab === "stress" && (
              <div className="rounded-2xl bg-white/80 dark:bg-ink/80 border border-ink/5 dark:border-cream/10 p-5">
                <div className="text-[10px] uppercase tracking-[0.25em] font-jakarta font-semibold text-coral mb-2">
                  Operational health
                </div>
                ...
                <p className="text-xs text-ink/55 dark:text-cream/55 font-jakarta mt-1">
                ...
                <div className="mt-5 flex items-center gap-5 flex-wrap">
                  ...
                  <div className="flex-1 min-w-[240px]">
                    {(stress?.signals || []).length === 0 ? (
                      <div className="rounded-xl bg-mint/40 border border-white dark:border-cream/10 p-3">
                      ...
                      <ul className="space-y-2">
                        {(stress?.signals || []).map((s) => (
                          <li
                            key={s.key}
                            className="rounded-xl bg-cream dark:bg-ink/60 border border-ink/5 dark:border-cream/10 p-3"
                          >
      ...
      <ConfirmDialog
        ...
        message={
          deleteError ? (
            ...
            <>
              "{deleteTarget.title}" will be removed from your shop. Buyers
              won't see it anymore. This action can't be undone.
            </>
          ) : null
        }
        confirmLabel="Delete"
...
function KpiCard({ icon, label, value, tone }) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`rounded-2xl p-4 border border-ink/5 dark:border-cream/10 bg-gradient-to-br ${tone}`}
    >
...
function SellerSettingsTab({ user, onSaved }) {
  ...
  return (
    <form
      onSubmit={save}
      className="max-w-2xl rounded-2xl bg-white/80 dark:bg-ink/80 border border-ink/5 dark:border-cream/10 p-5 space-y-4"
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] font-jakarta text-ink/55 dark:text-cream/55 mt-0.5">
      ...
      <div>
        ...
        <div className="flex items-stretch gap-2">
          <input
            value={user.email || ""}
            readOnly
            className="flex-1 rounded-xl bg-ink/5 dark:bg-white/5 border border-ink/10 dark:border-cream/10 outline-none px-3 py-2.5 text-xs text-ink/70 dark:text-cream/70 font-jakarta cursor-not-allowed"
          />
          {!user.emailVerified && (
            <Button
              type="button"
          ...
          {user.emailVerified && (
            <span className="inline-flex items-center gap-1 text-[11px] font-jakarta font-semibold text-leaf px-2">
      ...
      <div className="pt-2 border-t border-ink/5 dark:border-cream/10">
...
function AdminPanel() {
  ...
  return (
    <div className="rounded-2xl bg-white/80 dark:bg-ink/80 border border-ink/5 dark:border-cream/10 p-5 space-y-3">
      <div className="text-[10px] uppercase tracking-[0.25em] font-jakarta font-semibold text-coral mb-2">
        Operator tools
      </div>
      <h3 className="font-fraunces text-xl text-ink dark:text-cream tracking-tight flex items-center gap-2">
