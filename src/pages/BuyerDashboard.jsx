...
function OverviewTab({ user, setTab }) {
  ...
  return (
    <div className="grid md:grid-cols-4 gap-3">
      ...
      <div className="md:col-span-2 rounded-2xl bg-white/80 dark:bg-white/5 border border-ink/5 dark:border-white/10 p-4">
        ...
        {orders.length === 0 && (
          <p className="mt-2 text-xs text-ink/50 dark:text-cream/50 font-jakarta italic">
            No orders yet — <Link to="/products" className="text-coral">browse products</Link>
          </p>
        )}
      </div>
      ...
      {orders.map((o) => {
        ...
        return (
          ...
          >
            ...
            <Link
              to={`/order/${o._id}`}
              className="text-[11px] font-jakarta font-semibold text-ink/70 dark:text-cream/70 hover:text-coral transition"
            >
              Track →
            </Link>
            {delivered && (
              <Link
                ...
                className="text-[11px] font-jakarta font-semibold rounded-full px-3 py-1 bg-coral text-white hover:opacity-90"
              >
                Review
              </Link>
            )}
...
function WishlistTab() {
  ...
  const [loading, setLoading] = useState(true);
  // Probe backend /wishlist once. If 404, we fall back to the local-storage
  // wishlist managed on product pages. If supported, we prefer the server list.
  const [serverSupported, setServerSupported] = useState(null); // null|true|false
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(({ data }) => {
        ...
        // Items can be product objects or { product: {...} } wrappers.
  ...
  useEffect(() => {
    if (serverSupported !== false) return; // only hydrate local wishlist when no backend
    if (ids.length === 0) {
      setProducts([]);
      return;
    }
...
function CoinsTab({ user }) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl bg-gradient-to-br from-butter to-peach p-5 border border-ink/5 flex items-center gap-4">
        <div className="w-12 h-12 rounded-xl bg-white/70 grid place-items-center text-tangerine text-xl">
          <TbCoins />
        </div>
        ...
        <Link
          to="/coins"
          className="text-[11px] font-jakarta font-semibold rounded-full px-3 py-1.5 bg-ink text-cream hover:opacity-90"
        >
...
function ReviewsTab({ user }) {
  ...
  useEffect(() => {
    // Try /reviews/mine first; fall back to /reviews?buyer=<me>; finally ?buyer=me.
    async function load() {
      try {
        ...
        if (e1.response?.status !== 404) {
          // fall through to try the query-param form
        }
      }
      try {
      ...
      {reviews.map((r) => (
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            <HiOutlineStar className="text-butter" />
...
function ChatsTab() {
  ...
  return (
    <div className="rounded-2xl bg-white/80 dark:bg-white/5 border border-ink/5 dark:border-white/10 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-ink/5 dark:border-white/10">
            ...
            <li key={c.id}>
              <div className="flex items-center gap-3 p-3 hover:bg-peach/40 dark:hover:bg-white/5 transition">
                ...
                <Link
                  ...
                  className="text-[11px] font-jakarta font-semibold rounded-full px-3 py-1 bg-ink text-cream hover:opacity-90"
                >
                  Open
                </Link>
              </div>
...
function SettingsTab({ user, onSaved }) {
  ...
  return (
    <form
      onSubmit={save}
      className="max-w-2xl rounded-2xl bg-white/80 dark:bg-white/5 border border-ink/5 dark:border-white/10 p-5 space-y-4"
    >
      <div className="flex items-center gap-4">
        <MediaUploader
          value={avatar}
        ...
        />
        <div className="flex-1 min-w-0">
          <div className="font-fraunces text-lg text-ink dark:text-cream tracking-tight">
            {name || "Your profile"}
          </div>
