    ...
    <div className="max-w-6xl mx-auto px-4 md:px-8 py-8 md:py-10">
      ...
      {!supported && (
        <div className="mt-5 rounded-xl bg-butter/60 border border-butter/40 p-3 flex items-start gap-2">
          ...
          <p className="text-[11px] font-jakarta text-ink/75 leading-relaxed">
      ...
      <div className="mt-10 flex flex-col items-center">
        <div className="relative grid place-items-center">
          ...
          <motion.button
            ...
            className={`relative w-24 h-24 rounded-full grid place-items-center text-white transition-colors ${
              listening ? "bg-coral" : "bg-ink hover:bg-ink/90"
            }`}
        ...
        <div className="mt-4 text-[10px] uppercase tracking-[0.25em] font-jakarta font-semibold text-ink/50">
          {listening ? "Listening..." : "Tap to speak"}
        </div>

        {/* Language picker */}
        <div className="mt-5 inline-flex items-center gap-0.5 bg-white/70 border border-ink/5 rounded-full p-1">
          {LANGS.map((l) => (
            <button
              key={l.code}
      ...
      <div className="mt-8 min-h-[60px] flex items-center justify-center">
        {transcript ? (
          ...
          >
            <div className="text-[10px] uppercase tracking-[0.2em] font-jakarta font-semibold text-ink/45 mb-2">
              Transcript
            </div>
            <div className="font-fraunces text-xl md:text-2xl text-ink tracking-tight leading-tight max-w-2xl">
              "{transcript}"
            </div>
          </motion.div>
        ) : (
          <div className="text-center">
            <div className="text-[10px] uppercase tracking-[0.2em] font-jakarta font-semibold text-ink/40 mb-2">
              Try saying
            </div>
            <div className="space-y-1">
              {EXAMPLE_QUERIES.map((q, i) => (
                <div
                  key={i}
                  className="text-xs font-jakarta text-ink/55 italic"
                >
                  "{q}"
                </div>
              ))}
